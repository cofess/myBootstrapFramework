!function ($) {
	$.fn.extend({
		tree: function(data, options){
			...
			var $a = $("<a class='tree-icon fa'>").appendTo($li);
			...
			// Generate as list
			if(_list.length) {
				var cls_folder_open = options.folderOpenIcon || 'fa-folder-close';
				var cls_folder_close = options.folderCloseIcon || 'fa-folder-open';
				...
				var cls_file = options.itemIcon || 'fa-file';

	...
	$('.github-widget').each(function(){
		...
		$.ajax({
			...
			success: function(results) {
                ...
                repo.language!=null&&$widget.find('.updated').append('<p>开发语言：' + repo.language + '</p>');
		...
		$.ajax({
			url: 'https://api.github.com/repos/' + repo + "/issues?state=open&per_page=5&page=1&sort=updated",
			dataType: 'jsonp',
			success: function(results) {
				var repo = results.data, date, pushed_at = 'unknown';
                ...
                $widget.find('.issues').append('<tr><td colspan="2"><a href="' + repo.html_url + '/issues" target="_blank"><strong>问题列表：</strong></a></td></tr>');
					for(var i=0;i<repo.length;i++) {
						var updated_at = repo[i].updated_at.substr(0,10);
						$widget.find('.issues').append('<tr> \
							<td class="number" width="30">#' + repo[i].number + '</td> \
							<td class="info"><a href="' + repo[i].html_url + '" target="_blank">' + repo[i].title + '</a></td> \
							<td class="author" width="200" align="right">由 <a href="' + repo[i].user.url.replace('api.','').replace('users/','') 
							+'" target="_blank">' + repo[i].user.login
